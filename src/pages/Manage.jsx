...
export default function Manage() {
  ...
  const [editingId, setEditingId] = useState(null); // null = tambah baru, selain itu = edit
          ...
          <form onSubmit={onSubmit} className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="md:col-span-2 lg:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-4">
              <label className="flex items-center gap-3">
                <input type="radio" name="imgMode" value="url"
                  checked={imgMode === "url"} onChange={() => setImgMode("url")} />
                <span>Gunakan URL</span>
              </label>
              <label className="flex items-center gap-3">
                <input type="radio" name="imgMode" value="upload"
                  checked={imgMode === "upload"} onChange={() => setImgMode("upload")} />
                <span>Upload & kompres (disimpan lokal)</span>
              </label>
            </div>

            {imgMode === "url" ? (
              <Input className="md:col-span-2 lg:col-span-3"
                label="Foto sampul (URL)"
                name="foto_sampul" value={form.foto_sampul} onChange={onChange}
                placeholder="/content-img/new_1.webp atau https://..." required
              />
            ) : (
              <div className="md:col-span-2 lg:col-span-3">
                <label className="flex flex-col gap-1">
                  <span className="text-sm text-white/80">Upload gambar (akan diresize: poster 234×365, history 302×162)</span>
                  <input type="file" accept="image/*"
                    onChange={onPickFile}
                    className="rounded-xl border border-white/20 bg-transparent px-4 py-2" />
                  <input type="hidden" name="foto_sampul" value={form.foto_sampul} />
                </label>
              </div>
            )}
            ...
            {form.foto_sampul && (
              <div className="md:col-span-2 lg:col-span-3">
                <p className="text-sm mb-2 text-white/70">Preview:</p>
                <img src={form.foto_sampul} alt="preview"
            ...
            <button className="rounded-full bg-[#0F1E93] hover:bg-[#0E1A7A] px-6 py-2 font-semibold">
