...
export default function SeriesHero({ genres = [], onGenreChange }) {
  ...
  return (
    <section className="relative w-full h-[64vh] sm:h-[70vh] max-h-[587px] bg-no-repeat bg-cover overflow-hidden"
      style={{
    ...
    >
      ...
      <div ref={dropRef} className="absolute z-30" style={{ top: "60px", left: "80px" }}>
        ...
        {open && (
          <div className="absolute mt-3 p-4 z-40
          ...
          >
            {genres.map((g) => (
              <button key={g} onClick={() => pick(g)} className="text-left text-sm hover:text-[#9DA0A1] transition-colors">
                {g}
              </button>
            ))}
          </div>
